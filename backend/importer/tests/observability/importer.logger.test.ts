...
import {
  importerLogger,
} from "../../src/observability/importer.logger.js";
...
describe("importerLogger", () => {
  ...
  it("writes warning messages to console.warn", () => {
    ...
    expect(consoleWarn).toHaveBeenCalledTimes(
      1,
    );
  ...
  it("supports messages without context", () => {
    ...
    expect(consoleLog).toHaveBeenCalledTimes(
      1,
    );
  ...
  it("produces valid JSON for every log level", () => {
    ...
    expect(() =>
      JSON.parse(
        consoleLog.mock.calls[0]?.[0] as string,
      ),
    ).not.toThrow();

    expect(() =>
      JSON.parse(
        consoleWarn.mock.calls[0]?.[0] as string,
      ),
    ).not.toThrow();

    expect(() =>
      JSON.parse(
        consoleError.mock.calls[0]?.[0] as string,
      ),
    ).not.toThrow();
  });
});